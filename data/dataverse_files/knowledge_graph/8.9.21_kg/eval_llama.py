...
def predict_gene(prompt, model, tokenizer):
    """Predicts the true gene using the Llama 3.1 model."""
    ...
    # This might need adjustment based on the model's output format
...
def process_patient_file(filepath, hpo_to_name_dict, hpo_to_idx_dict, ensembl_to_idx_dict, kg_node_map, kg_edgelist, model, tokenizer, num_patients=None):
...
if __name__ == "__main__":
    ...
    results, accuracy = process_patient_file(filepath, hpo_to_name_dict, hpo_to_idx_dict, ensembl_to_idx_dict, kg_node_map, kg_edgelist, model, tokenizer, num_patients=10)
